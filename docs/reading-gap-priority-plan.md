...
| P8 | 부분 구현 | AI runtime provenance 표시 | 생성 성공 여부와 runtime 오류를 구분한다. |
...
- runtime-generated 결과와 runtime error 상태가 구분된다.
...
- **Description**: summary/chat 결과에 `generatedBy`, `runtimeUsed`, `runtimeError` 같은 provenance metadata를 저장한다.
- **Dependencies**: 없음
- **Acceptance Criteria**:
  - runtime success/error가 session에 남는다.
  - legacy session은 기본값으로 정규화된다.
- **Validation**:
  - runtime unavailable test에서 생성물이 저장되지 않는지 확인.

### Task 4.2: Provenance UI 표시

- **Location**: `web/app/features/reading.js`, `web/styles.css`
- **Description**: Summary/Chat footer에 runtime 생성 여부와 오류 상태를 작게 표시한다.
- **Dependencies**: Task 4.1
- **Acceptance Criteria**:
  - 사용자가 결과의 생성 방식을 구분할 수 있다.
  - UI가 과하게 시끄럽지 않다.
- **Validation**:
  - runtime unavailable session과 runtime success session 각각 상태 표시 확인.
...
- **Validation**:
  - reading-service test에 selected text/PDF location prompt contract 추가.

### Task 4.4: Agent chat prompt contract

- **Location**: `services/backend/lib/reading-service.mjs`, `web/app/features/reading.js`
- **Description**: agent에는 사용자 질문, 선택한 PDF 텍스트, PDF 위치만 전달한다.
- **Dependencies**: Task 4.3
- **Acceptance Criteria**:
  - chunk 검색 결과, 점수, telemetry가 chat prompt와 assistant message에 남지 않는다.
  - 선택 텍스트가 있으면 prompt에 그대로 포함된다.
- **Validation**:
  - unrelated question test 추가.

## Sprint 5: 부분 구현 제품화 3차 - Highlight 품질과 session lifecycle

**Goal**: parse highlight와 session 관리 기능을 안정화한다. Notes는 사용자 작성 산출물로만 유지한다.

**Demo/Validation**:

- parse highlight가 더 예측 가능하게 생성된다.
...
- Runtime-generated AI and runtime error states can look similar if inferred later. Provenance should be stored at generation time, not inferred later in UI.
